...
try{
    ...
    if($conn->connect_error){
        ...
        if(mysqli_fetch_row($checker) > 0){
            ...
            $stmt = $conn->prepare("insert into generate(batch, mixer, sku, keg, date, time, operator, bin, keg_1, keg_2, keg_3, keg_4, keg_5, keg_6, keg_7, keg_8, keg_9, keg_10, keg_11, keg_12) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssssssssssssss", $batch, $mixer, $sku, $keg, $date, $time, $operator, $bin, $kegsArray_1, $kegsArray_2, $kegsArray_3, $kegsArray_4, $kegsArray_5, $kegsArray_6, $kegsArray_7, $kegsArray_8, $kegsArray_9, $kegsArray_10, $kegsArray_11, $kegsArray_12);
