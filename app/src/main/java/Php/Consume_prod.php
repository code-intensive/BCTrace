...
try{
    ...
    if($conn->connect_error){
        ...
        $stmt = $conn->prepare("insert into consume_prod(batch, mixer, sku, keg, c_date, c_time, operator, bin, dateResult, timeResult, usages, machine, consumed_keg) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmt->bind_param("sssssssssssss", $batchNo, $mixer, $sku, $kegNo, $c_date, $c_time, $operator, $bin, $dateResult, $timeResult, $usageType, $machine, $consumed_keg);
